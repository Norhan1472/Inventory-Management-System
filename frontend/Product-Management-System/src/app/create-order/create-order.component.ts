...
export class CreateOrderComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.product.push(new Product('',null,null));
  ...
  addProduct() {
    ...
    rollBackToAllOrders(){
      this.router.navigate(["getAllOrders"]);
    }
  onSubmit(){
    console.log(this.productArray.value);
    this.orderService.addOrder(this.orders).subscribe(
      data=>{
        console.log(data);
        console.log("xxxxxxxxxxxxx1234");
        console.log(this.productArray.value);
        this.loopOnProductArrray(data.orderId);
      },
      error=>{
        console.log(error);
      }
